...
Rain checks for a new frame every `rain_poll_seconds`, replacing the earlier fixed schedule that fired just after each five-minute slot. Each check reads the official daily catalogue item for the current UTC day and takes the newest listed RZC frame, whenever it was published. It reads the previous day only when the current day lists nothing usable, which covers frames just after midnight. There is no archive walk: rain can only use a frame newer than `rain_max_age_minutes`, so a listed older frame is reported as `stale` and never downloaded.

Rain never builds a file name. MeteoSwiss names these grids `RZCyyjjjHHMMKK.XYZ.h5`, and the two-character `KK` follows radar-site availability. It was `nl`, then `vl`, and on 2026-09-15 every published frame was `ul` while the built `vl` name returned 403. Only the catalogue knows the published name, so rain requests the listed href and verifies the listed SHA256 before decoding.

Observation timestamps come from the source file name, never from the poll or the STAC item datetime. A frame is identified by its URL and checksum, so an unchanged frame isn't downloaded again but a same-time correction is. A poll that finds nothing new keeps the cached observation and its original timestamp; freshness is re-evaluated on every entity read and published again by an expiry timer at `observation + rain_max_age_minutes`, without a download. Age keeps advancing through missing and failed periods because failures are reported as health instead of raising, so the coordinator keeps publishing and polling.

Absence is a daily item that lists no usable frame, or a daily-item 404 after its bounded retries. Every other status, including 403 on the file itself, plus redirects, malformed catalogue responses, transport failures, timeouts, checksum mismatches and decoding errors, is `error` rather than absence, and stays latched until a download succeeds. An absent frame can't restore a previous reading. Re-fetching the cached frame is a valid recovery and keeps its original timestamp. Catalogue parsing, decoding and the cell search run in an executor, off the event loop. Responses are size-bounded, requests don't follow redirects, and at most two daily items are cached, revalidated with ETag or `Last-Modified` on later polls. Unloading cancels both timers and starts no new request; a request already in flight may finish, but it can't rearm a timer.
